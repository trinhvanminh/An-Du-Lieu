...
def decode_scan(r, image, scan):
    ...
    for mcu_idx in range(nmcu[0] * nmcu[1]):
        ...
        for ci, bx, by in blocks:
            ...
            blkimg, pred[ci], blk_quant_dct_coefs = decode_block8x8(r, hdec[ci * 2:ci * 2 + 2], pred[ci], qtbl[ci], bpp)
            quant_dct_coefs = np.append(quant_dct_coefs, blk_quant_dct_coefs)# ***
